...
export default function VendorReviews({ vendorId, vendorName }: VendorReviewsProps) {
  ...
  const checkIfUserHasReviewed = async () => {
    ...
    try {
      const response = await fetch(`/api/v1/vendor/reviews/check?vendorId=${vendorId}&userId=${session.user.uid}`);
      if (response.ok) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-gray-900">Vendor Reviews</h2>
